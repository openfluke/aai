...
test54_parse_layer_args() {
  ...
  TEST54_REMAINING_ARGS+=("$@")
  if ! [[ "$TEST54_CAMS" =~ ^[0-9]+$ ]] || [[ "$TEST54_CAMS" -lt 1 ]]; then
    echo "error: TEST54_CAMS must be ≥1, got $TEST54_CAMS" >&2
    exit 1
  fi
  export TEST54_LAYER TEST54_CAMS TEST54_LAYERS="$TEST54_LAYER"
}

# Host Tide port (offset from test53): cam1 lo=9080 neg=9081 hi=9082; cam3 lo=9100 …
test54_tide_port() {
  local cam="${1:-1}"
  local band="${2:-lo}"
  local off=0
