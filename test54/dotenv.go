...
// resolveCkpt puts single-layer native runs under test54_ckpt/<layer>/.
// Docker sets TEST54_CKPT_FLAT=true and bind-mounts the layer dir directly.
func resolveCkpt(base string, kinds []CellKind) string {
	if EnvBool("TEST54_CKPT_FLAT", false) {
		return base
	}
	if len(kinds) != 1 {
		return base
