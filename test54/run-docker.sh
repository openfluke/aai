#!/usr/bin/env bash
# test54 — Podman-first (Mac + Linux). Compiles Go binary, then compose up.
#
...
mkdir -p "$TEST54_CKPT_HOST" "$BIN_DIR"
...
if [[ -n "${TEST54_COMPOSE_OVERRIDE:-}" ]]; then
  compose_args+=(-f "$DIR/$TEST54_COMPOSE_OVERRIDE")
fi
...
compile_binary() {
  ...
  run_compile() {
    drun --rm "$@" \
      ...
      bash -ec '
        ...
        go build -buildvcs=false -trimpath -ldflags="-s -w" -o /out/test54 .
        ls -lh /out/test54
      '
  }
  ...
  if [[ ! -f "$BIN_DIR/test54" ]]; then
    echo "error: compile failed — no $BIN_DIR/test54" >&2
    exit 1
  fi
  echo "binary ready: $(du -h "$BIN_DIR/test54" | awk '{print $1}') → compose context .bin/"
}
...
case "$cmd" in
  up|""|--build|build)
    ...
    if command -v lsof >/dev/null 2>&1 && [[ "${TEST54_FORCE_PORT:-}" == "1" ]]; then
      pids=$(lsof -tiTCP:"${TIDE_PORT}" -sTCP:LISTEN 2>/dev/null || true)
      if [[ -n "${pids:-}" ]]; then
        echo "TEST54_FORCE_PORT=1 — killing host listeners on :${TIDE_PORT} → $pids"
      ...
      pids=$(lsof -tiTCP:"${TIDE_PORT}" -sTCP:LISTEN 2>/dev/null || true)
      if [[ -n "${pids:-}" ]]; then
        echo "warn: :${TIDE_PORT} already in use (pid $pids) — not killing (set TEST54_FORCE_PORT=1 to override)"
        echo "      another cam/band may be running; use ./status-all.sh to check"
      fi
    fi
    echo "engine=$ENGINE  project=$PROJECT  lrs=$TEST54_LRS"
    if [[ "$cmd" == "--build" || "$cmd" == "build" ]]; then
      compile_binary
      dc up --build -d
    else
      if [[ ! -f "$BIN_DIR/test54" ]]; then
        echo "no .bin/test54 yet — compiling once"
        compile_binary
      fi
      dc up -d
    fi
  ...
  *)
    if [[ "$cmd" != up && "$cmd" != --* && "$cmd" != build ]]; then
      ...
      echo "  (git pull if you still see ckpt=test54_ckpt_hi without /dense/)" >&2
    fi
