#!/usr/bin/env bash
# Stop test54 hi farm(s). Default cam1; cam3 / both supported.
#   ./stop-hi.sh
#   ./stop-hi.sh cam3
#   ./stop-hi.sh both
set -euo pipefail
...
for cam in "${TEST54_CAMS_LIST[@]}"; do
  ...
  export TEST54_CKPT_HOST="$(test54_ckpt_host_for_layer "$(test54_ckpt_root_for_band hi "$cam")" "$TEST54_LAYER")"
  export TEST54_COMPOSE_OVERRIDE=docker-compose.hi.yml
  echo "stopping HI cam=$cam project=$TEST54_PROJECT"
