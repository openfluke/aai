...
const (
	...
	defaultHidden  = 32
	defaultDepth   = 4 // mid blocks between stem and head (stem+4 mids+head = 6 stages)
...
func main() {
	...
	resume := flag.Bool("resume", EnvBool("TEST54_RESUME", true), "skip done IDs")
	tideAddr := flag.String("tide-addr", EnvOr("TEST54_TIDE_ADDR", "0.0.0.0:8080"), "Tide dash (empty=off)")
	flag.Parse()

	modes, err := parseModeList(*modesFlag)
	must(err)
	kinds, err := parseLayerList("mamba", *layersFlag, nil)
	must(err)
	...
	fmt.Println("╔══════════════════════════════════════════════════════════════╗")
	fmt.Println("║  test54 · deep dayroute × layer × mode × dtype × LR         ║")
	fmt.Println("╚══════════════════════════════════════════════════════════════╝")
	...
	fmt.Printf("stack=Dense→mid×%d→Dense  hidden=%d  stages=%d\n", *depth, *hidden, *depth+2)
	...
	for w := 0; w < workers; w++ {
		...
		go func(seed int64) {
			...
			for j := range jobCh {
				...
				t0 := time.Now()
				res := runJob(j, seed, *hidden, *depth, nCams, *dur)
...
func runJob(j Job, seed int64, hidden, depth, nCams int, dur time.Duration) modeResult {
	lr := j.LR
	r := modeResult{
		ID: j.ID, Layer: string(j.Kind), DType: j.DType.String(), Mode: j.Mode.String(), LR: lr,
	}
	st, err := buildSandwich(j.Kind, obsDim, hidden, nActs, depth, nCams, j.DType)
	if err != nil {
		r.Err = err.Error()
		return r
	}
...
func printTopLPD(results []modeResult, n int) {
	...
	fmt.Printf("\ncompleted ok=%d fail=%d → test54_ckpt/lpd.json\n", ok, fail)
}
