...
// TEST 46 — ARC-AGI native camerals with whole-net StepTweenSplit
//
// Same sandwich / Lucy loop as test44, but training is Tween not TweenChain:
// one output gap, split 1/N across every trainable leaf (stem, each
// hemisphere, head). No chain rule. Default sweep is 1-cameral through 8.
...
func main() {
	...
	camerals := flag.Int("camerals", 8, "max native hemispheres (sweeps cam-min..N plus Dense)")
	camMin := flag.Int("cam-min", 1, "first cameral count (inclusive; 1 = Dense / single mid)")
	...
	mode := parallel.ModeStepTweenSplit
	dim := vecDim(MaxGrid)
	...
	fmt.Println("╔═════════════════════════════════════════════════════════════════════════════════════╗")
	fmt.Println("║   TEST 46 — whole-net TweenSplit (not TweenChain)  |  1..8-cameral default          ║")
	fmt.Println("║   One output gap split across stem + hemispheres + head  |  no chain rule           ║")
	fmt.Println("╚═════════════════════════════════════════════════════════════════════════════════════╝")
	fmt.Printf("\n📂 train %s\n📂 eval  %s\n", trainDir, evalDir)
	...
	fmt.Printf("   arches:")
	sep := " "
	if !*noDense && containsKind(kinds, KindDense) && *camMin > 1 {
		fmt.Printf(" Dense")
		sep = " +"
	}
	for n := *camMin; n <= *camerals; n++ {
		fmt.Printf("%s %s", sep, camName(n))
		sep = " +"
	}
	...
	results := &RunResults{
		Engine:       "welvet-v0.95.1-tween-split",
...
func saveResults(results *RunResults) {
	...
	_ = os.WriteFile("test46_results.json", data, 0644)
	fmt.Println("\n✅ Results saved to test46_results.json")
}
