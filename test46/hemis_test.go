...
func TestParseLayersAllSkipsDense(t *testing.T) {
...
func TestHemisphereSandwichTrain(t *testing.T) {
	...
	kinds := append([]CellKind{KindDense}, allKindsExceptDense()...)
	for _, kind := range kinds {
		kind := kind
		t.Run(string(kind), func(t *testing.T) {
			stack, err := buildNativeCameral(kind, dim, hidden, dim, 3, parallel.ModeStepTweenSplit)
			if err != nil {
			...
			loss, err := parallel.TrainStackMSE(stack, x, y, parallel.ModeStepTweenSplit, 0.01)
			if err != nil {
